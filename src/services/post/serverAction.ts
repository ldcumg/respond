...
/** 사진 추가하기 */
export const imagePosting = async ({ user_id, board_id, bucketData }: PostImage) => {
  const supabase = createClient();

  const { path } = bucketData;

  const { publicUrl } = supabase.storage.from("board_img").getPublicUrl(path).data;

  return await supabase.from("board_img").insert({ user_id, board_id, img_url: publicUrl });
};

/** 해당 유저의 게시물 모두 불러오기 */
export const getPosts = async (userId: string) => {
  const supabase = createClient();
  const { data, error } = await supabase.from(board).select().eq("user_id", userId);
