  ...
  return (
    <div className="p-6">
      ...
      <div className="flex justify-between gap-5">
        <div className="border-3 mb-4 w-1/3 rounded-[15px] border-solid border-black p-4">
      ...
      <div>
        <div className="mt-4 flex items-center justify-center">
          <input
            ...
            className="border-3 h-10 w-3/5 rounded-[15px] border-black"
            maxLength={15}
          />
          ...
          <ul>
            {currentTodos.map((todo, index) => (
              <li key={index} className="flex items-center justify-between divide-y divide-dashed">
                {todo.내용} - {todo.날짜}
                <div className="gap-5">
                  <button onClick={() => handleCompleteTodo(startIndex + index)} className="mr-2 rounded px-2 py-1">
                    완료
                  </button>
                  <button onClick={() => handleCancelTodo(startIndex + index)} className="rounded px-2 py-1">
                    취소
                  </button>
                </div>
              </li>
